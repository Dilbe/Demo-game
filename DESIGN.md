...
**v7 status: done** (6 of 7 milestones — milestone 6 was deliberately skipped, see above). The Skills tab list and slot bar both render unlocked skills as squares styled like the in-combat buttons; clicking either opens a shared detail panel (a right-hand sidebar at desktop widths) with that skill's stats and upgrade tracks. Equipping past the Skill Points budget shows a specific shortfall message instead of silently failing. Kills inside a multi-monster group compound ×1.25 per kill (rounded up, computed fresh from each kill's own base XP rather than chained), and a dungeon's `completionBonusXp` pays out once on a full clear — both folded into the "X XP total" previews so they match what's actually paid out. Each combat skill button shows a fixed line marking exactly where its `triggerAt` fraction lands as the cooldown fill drains.
...
**Milestones 3-10 status: done.** Shipped together in one PR — same reasoning as v8: the toggle system (milestones 4–7, 9–10) is one coherent feature, not independently useful in pieces. `OBJECTIVES` is a new data-object map alongside `QUESTS`, deliberately not folded into it: a quest completes strictly in sequence with one "active" one shown in the tracker, while `killMedium`/`killBig`/`winDungeon` can each complete in any order the moment their `condition` matches an event (`registerObjectiveEvent`, called on every kill and every full dungeon clear) — so nothing needed to change about how QUESTS or the tracker work. Strong Attack and Heal dropped `unlockCost` entirely in favor of `unlockObjectiveId`; completing the matching objective unlocks and auto-equips them the same way buying a skill with XP always has (`markSkillUnlocked`, factored out of `unlockSkill` so both paths share it). Each skill's new `toggles` array (separate from the continuously-leveled `upgrades`) is a one-time XP unlock that then switches on/off freely, adding its `pointSurcharge` to the skill's Skill Point cost only while on (`effectivePointCost`, which `pointsUsed`/`canEquip`/`equipInSlot` all route through now instead of reading `pointCost` directly) — gated as a whole behind the `winDungeon` objective, visible but unpurchasable/unswitchable until then. Multi Attack (Basic Attack) hits every monster still standing for full damage instead of just the target; Heal over Time (Heal) spreads the same total healing over 10 one-second ticks instead of landing it all at once, each tick sized off the difference between successive rounded running totals so the total heals exactly right regardless of rounding. Auto-Trigger (Basic Attack and Strong Attack) replaces Auto Attack's old static `auto: true` field with a live per-skill toggle check (`isAutoTriggering`), which is how Auto Attack itself got removed outright rather than kept as a third automatic option — its old unlock/equip cost pattern is fully superseded by the toggle. A save from before this shipped that had Auto Attack unlocked/equipped loads cleanly: it's filtered out of `unlockedSkills`/`equippedSkills` and never merges into `skillLevels` on load, verified by loading a hand-built old-shaped save and confirming no dangling reference remained. Verified live: killing a Medium/Big Monster unlocks Strong Attack/Heal and auto-equips them when a slot is free, winning a dungeon flips every skill's toggle list from locked to purchasable, Multi Attack damages both monsters in a Two Small Monsters fight from one Basic Attack, Auto-Trigger fires Basic Attack with no click needed, and Heal over Time visibly heals in increments across 10 seconds rather than all at once.

**v9 status: done.** All 10 milestones complete.
...
**v10 status: done.** Both milestones complete. Monsters were renamed alongside their sprites so the two actually match — Small Monster/Medium Monster/Big Monster became Small Slime/Goblin/Orc (`MONSTERS`' own ids, and `MONSTER_GROUPS`' ids, are untouched — only the user-facing `label`s changed, so no save data referencing them by id breaks); `OBJECTIVES.killMedium`/`killBig`'s descriptions were updated to match ("Kill a Goblin"/"Kill an Orc"). Each `MONSTERS` entry got a `sprite` field and each `SKILLS` entry an `icon` field — both plain inline SVG strings (simple shapes: circles, rects, polygons — no illustration, no image files), rendered via a small `buildSprite` helper that wraps the markup in a `<span>` via `innerHTML` (safe here since every string is our own hardcoded data, never anything a player supplies). Monster sprites appear on the selection screen's monster-group cards (the group's first monster represents it — a dungeon card, chaining different monsters, gets no sprite) and on each combatant card during a fight; skill icons appear on the in-combat button, the Skills-tab square (locked or not), and the equipped-slot box. `.cooldown-button` became a flex column so an icon and its label stack cleanly within the existing 80×80 box, and icons draw in `currentColor` so they always match whatever text color the button/square/slot already uses. Verified live at both a 1000px and a 375px viewport: all three monster sprites and all five skill icons render distinctly, no horizontal overflow, and `node --test` passes all 78 tests (2 new, checking every sprite/icon string is well-formed).
...
**Milestone 1 status: done.** Added the `viewport` meta tag so mobile browsers render at device width instead of a zoomed-out desktop layout. The multi-column rows that assumed a wide screen (`.monster-row`, `.stat-row`, `.skill-row`, plus the `#skill-bar`/`#skill-slots` button rows) now wrap instead of forcing horizontal scroll, and their fixed-width text columns (`.monster-detail`, `.stat-description`, `.stat-change`, `.skill-detail`) became `flex` items with a `max-width` cap so they shrink on a narrow viewport rather than holding a row open past the screen edge. `#health-bar`, `#quest-tracker`, and `#skill-detail-panel` switched from a fixed `width` to `width: 100%` (or `flex: 1 1 …`) with the old value kept as `max-width`, so they still cap out at their desktop size but shrink below it instead of overflowing. `#skills-main` needed `flex: 1 1 auto; min-width: 0;` (it was `flex: 0 0 auto`, which let its content force the whole `#skills-layout` row wider than the viewport regardless of the wrapping added elsewhere) — a global `box-sizing: border-box` plus a small horizontal `body` padding round out the pass. Verified with Playwright at a 360px viewport: `document.documentElement.scrollWidth` matches `clientWidth` (no horizontal overflow) on the Fight tab (pre-fight selection and mid-fight skill bar), Character tab, Skills tab (including the locked-skill row and slot bar), and About tab; a 1280px-wide render confirms the desktop layout is pixel-identical to before. `node --test` still passes all 52 existing tests (CSS/HTML-only change, no formula code touched).

**Milestone 2 status: done.** Decided the approach flagged as still-to-be-decided in the v11 scope: tap-to-inspect-then-tap-a-slot, built as an *addition* to the existing detail panel rather than a replacement for drag-and-drop (so a desktop mouse loses nothing). Tapping an unlocked skill's square already opened its stats/upgrades panel (`inspectSkill`/`renderSkillDetail`, from v7); that panel now also gets a `buildEquipControls` block — one button per available slot, labelled with whatever currently occupies it (`Slot 2 (Strong Attack)`, `Slot 4 (Empty)`), disabled for the slot the skill is already in, and calling the same `equipInSlot` a drag-drop uses — plus an `Unequip` button when the skill is currently equipped anywhere, calling the same `unequipSkill`. Both reuse the existing functions verbatim, so the Skill Points budget check, the shortfall message, and the save-on-change behavior are identical between a drag and a tap; only a new way to invoke them was added. The hint text below the skill list now mentions the tap path alongside the drag one. Verified with Playwright using `hasTouch: true` and `.tap()`: tapping a skill opens its panel, tapping an empty slot's button equips it there, tapping a different skill's slot moves it (bumping the previous occupant, matching drag's swap behavior), and tapping Unequip clears it — all while the panel stays open and reflects the new state, and with no horizontal overflow at a 360px viewport. `node --test` still passes all 52 tests (no `formulas.js` change).

**Milestone 3 status: done.** Measured every button's rendered box with Playwright at a 375px viewport rather than guessing — confirmed the two examples DESIGN.md called out (`.upgrade-button`/`.unlock-button`, both unstyled and falling back to the browser default ~21px tall) plus every other text button in the game (`.tab-button`, `.monster-select-button`, `#start-button`/`#restart-button`/`#retreat-button`/`#reset-character-button`, and the milestone-2 `.equip-slot-button`/`.unequip-button`) measured 21–38px tall, short of the ~44px minimum comfortable tap target on both iOS and Android guidelines; only the pre-existing 80×80 `.cooldown-button` skill buttons already cleared it. Gave every one of those an explicit `min-height: 2.75rem` (44px) with `padding`/`display: inline-flex; align-items: center; justify-content: center` to keep the label centered rather than stretched, consolidating what had been either no rule at all or a bare `font-size`/`padding` pair. Also fixed `#tabs`, which laid its four buttons out with only the ~4px of whitespace HTML happens to leave between inline elements — now a `flex` row with a `0.6rem` gap (and wrapping, unchanged from milestone 1). Re-measured after the change: every button across the Fight tab (selection and mid-fight), Character tab, Skills tab (list, slots, and the detail panel's equip controls), and About tab is now ≥44px in both dimensions, with no new horizontal overflow at 360px and the desktop layout otherwise unchanged (just roomier buttons). `node --test` still passes all 52 tests.
...
**v12 status: done.** All 5 milestones complete, shipped together — same reasoning as v8/v9: Perk Points paying out but nothing yet to spend them on (or vice versa) would be invisible/dead work on its own. `prestigeCount(maxXp)` derives the prestige number the same way `prestigeTarget` already derives the bar's target — `(maxXp - STARTING_MAX_XP) / PRESTIGE_BONUS_PER_CYCLE` — so `prestige()` needs no counter of its own to award `prestigeCount(newMaxXp)` Perk Points (1st cycle pays 1, 2nd pays 2, ...). `PERKS` is a data-object map like every other config in `formulas.js`, each entry's `effect` a small `{ type, ...}` shape (`maxHp`, `healingSpeedPercent`, or `skillDamage` + a `skillId`) rather than code, read by three focused helpers (`perkMaxHpBonus`, `perkHealingSpeedMultiplier`, `perkSkillDamageBonus`) that mirror `passiveMultiplier`'s own pattern — Max HP perks sum additively, Healing Speed perks stack multiplicatively (so a future second one compounds rather than adds), and a skill-damage perk only ever matches the skill id it names. None of the three ever touch `stats`/`skillLevels` — they're read as an independent layer everywhere the thing they boost is read: `effectiveMaxHp()` (health bar, `healPlayer`'s cap, `regenTick`/`updateRegenIndicator`'s maxHp), `effectiveSecondsPerHp()` (already dividing by Regen's passive multiplier, now also by the Healing Speed perk's), and `applySkill`'s `basePower` (added before Strength's own passive multiplier applies to the total, and threaded into `describeSkill`'s optional third argument so the Skills-tab/detail-panel description shows the boosted number too). Perk Points and `purchasedPerkIds` live in `MAX_XP_KEY` alongside `maxXp` (`savePermanentProgress`/`loadProgress`, replacing the key's old bare-number shape with `{ maxXp, perkPoints, purchasedPerkIds }` — `JSON.parse` on an old save's plain number still round-trips cleanly, so no migration flag was needed) rather than `SAVE_KEY`, so a prestige's reset of everything else never touches them. The Character tab's new Perks section is always visible (even before a first prestige, so a new player can see up front what prestiging eventually buys), one row per perk showing its cost and description, a Buy button that becomes "Owned" once purchased and stays disabled while unaffordable. Verified live with Playwright: buying Max HP perks raises the health bar's displayed max immediately and survives a reload, stacks +10 and +25 additively to +35, and leaves `stats.maxHp` itself untouched (so the next XP-funded Max HP level still costs what it did); the Healing Speed perk divides `effectiveSecondsPerHp()` by exactly 1.25; the Basic Attack damage perk turns a 1-damage hit into 2 in a live fight; and prestiging twice in a row awards 1 then 2 more Perk Points (3 total) while raising Max XP 100→200→300.
